...
export const bookMiddleware = (state) => next => action => {
  next(action);
  ...
  switch (action.type) {
    ...
    case `${SEARCH_BOOKS} ${API_SUCCESS}`:
      if (action.payload.books && action.payload.books.error) {
        next(
          setNotification({
            message: action.payload.books.error,
            feature: SEARCH_BOOKS
          })
        );
      } else {
    ...
    case ADD_BOOK:
      console.log("ADD_BOOK",state.getState(),action);
      next(
        apiRequest({
    ...
    case `${ADD_BOOK} ${API_SUCCESS}`:
      next(
        setNotification({ message: '', feature: `${ADD_BOOK} ${API_SUCCESS}` })
      );
      next(setLoader({ state: false, feature: ADD_BOOK }));
      next(updateBook({ books: action.payload }));
      break;
